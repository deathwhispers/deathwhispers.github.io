...
![9f406b7cdcc672d1cff9e3474258686a](/assets/images/ai/computervision/deep-learning-fundamentals/9f406b7cdcc672d1cff9e3474258686a.png)
...
![6fbce4757f950bb2ff995ee22df8fbfa](/assets/images/ai/computervision/deep-learning-fundamentals/6fbce4757f950bb2ff995ee22df8fbfa.png)
...
![d1d82072dece0ce8ed2c719bdfdade9e](/assets/images/ai/computervision/deep-learning-fundamentals/d1d82072dece0ce8ed2c719bdfdade9e.png)
...
![2fc352b78cc727a6c1bf83c57ef4c000](/assets/images/ai/computervision/deep-learning-fundamentals/2fc352b78cc727a6c1bf83c57ef4c000.png)
...
![7f2ac1086f54ec8ccaa223826a31a5c3](/assets/images/ai/computervision/deep-learning-fundamentals/7f2ac1086f54ec8ccaa223826a31a5c3.png)
...
![7c65ef7cc666b99c51e53f4d3312632f](/assets/images/ai/computervision/deep-learning-fundamentals/7c65ef7cc666b99c51e53f4d3312632f.png)
...
![592f5423db0fc55515ae1cbe83d6873a](/assets/images/ai/computervision/deep-learning-fundamentals/592f5423db0fc55515ae1cbe83d6873a.png)
...
![115d69f3afbaad04a9011e6bae1d9bb4](/assets/images/ai/computervision/deep-learning-fundamentals/115d69f3afbaad04a9011e6bae1d9bb4.png)
...
![5f529a06a041ed7d7cabea057a253992](/assets/images/ai/computervision/deep-learning-fundamentals/5f529a06a041ed7d7cabea057a253992.png)
...
![81d785abbe7134a27a3e624c972b4601](/assets/images/ai/computervision/deep-learning-fundamentals/81d785abbe7134a27a3e624c972b4601.png)
...
![740847ba35663f0e3d52f45017afc769](/assets/images/ai/computervision/deep-learning-fundamentals/740847ba35663f0e3d52f45017afc769.png)
...
![8b76cfdebfd1103dbe90197a5066cb94](/assets/images/ai/computervision/deep-learning-fundamentals/8b76cfdebfd1103dbe90197a5066cb94.png)
...
![17406842333fcb03699c3b3d7fc4d3e1](/assets/images/ai/computervision/deep-learning-fundamentals/17406842333fcb03699c3b3d7fc4d3e1.png)
...
![31b6ba319140fad768ea964ac36ebe54](/assets/images/ai/computervision/deep-learning-fundamentals/31b6ba319140fad768ea964ac36ebe54.png)
...
![eb1dbcfdc8a4c4e604cfb21352c835f6](/assets/images/ai/computervision/deep-learning-fundamentals/eb1dbcfdc8a4c4e604cfb21352c835f6.png)
...
$$
LeakyReLU(x)=\max (0, x) + negative-slope * \min (0, x)
$$

![87066ef405e5c7d0ca700a241bfce3a0](/assets/images/ai/computervision/deep-learning-fundamentals/87066ef405e5c7d0ca700a241bfce3a0.png)
...
$$
\operatorname{Softmax}(x_{i})=\frac{\exp (x_{i})}{\sum_{j} \exp (x_{j})}
$$
...
![3ecfc4533c6871c621d1253d3dbc3276](/assets/images/ai/computervision/deep-learning-fundamentals/3ecfc4533c6871c621d1253d3dbc3276.png)
...
![6e09c00384efd3a8ddee0f481a03cf2b](/assets/images/ai/computervision/deep-learning-fundamentals/6e09c00384efd3a8ddee0f481a03cf2b.png)
...
![1e617320cd6a131d843dbc0a679afe9e](/assets/images/ai/computervision/deep-learning-fundamentals/1e617320cd6a131d843dbc0a679afe9e.png)
...
2. **可微性**：可微性保证了在优化中梯度的可计算性。传统的激活函数如 $Sigmoid$ 等满足处处可微。对于分段线性函数比如 $ReLU$ ，只满足几乎处处可微(即仅在有限个点处不可微)。对于 $SGD$ 算法来说，由于几乎不可能收敛到梯度接近零的位置，有限的不可微点对于优化结果不会有很大影响。
3. **计算简单**：非线性函有很多。激活函数在神经网络前向的计算次数与神经元的个数成正比，因此简单的非线性函数自然更适合用作激活函数。这也是 $ReLU$ 比其它使用 $Exp$ 等操作的激活函数更受欢迎的其中一个原因。
4. **非饱和性(saturation)**：饱和指的是在某些区间梯度接近于零(即梯度消失)，使得参数无法继续更新的问题。最经典的例子是 $Sigmoid$ ，它的导数在 $x$ 为比较大的正值和比较小的负值时都会接近于 0 。更极端的例子是阶跃函数，由于它在几乎所有位置的梯度都为 0 ，因此处处饱和，无法作为激活函数。 $ReLU$ 在 $x>0$ 时导数恒为 1，因此对于再大的正值也不会饱和。但同时对于 $x<0$ ，其梯度恒为 0，这时候它也会出现饱和的现象。 $Leaky ReLU$ 和 $PReLU$ 的提出正是为了解决这一问题。
...
7. **接近恒等变换(identity)**： $f(x)≈x$ ，即约等于 $x$。这样的好处是使得输出的幅值不会随着深度的增加而发生显著的增加，从而使网络更为稳定，同时梯度也能够更容易地回传。这个与非线性是有点矛盾的，因此激活函数基本只是部分满足这个条件。
8. **参数少**：大部分激活函数都是没有参数的。像 $PReLU$ 带单个参数会略微增加网络的大小。还有一个例外是$Maxout$，尽管本身没有参数，但在同样输出通道数下 $k$ 路 $Maxout$ 需要的输入通道数是其它函数的k倍，这意味着神经元数目也需要变为k倍；但如果不考虑维持输出通道数的情况下，该激活函数又能将参数个数减少为原来的k倍。
9. **归一化(normalization)**：对应的激活函数是 $SELU$ ，主要思想是使样本分布自动归一化到零均值、单位方差的分布，从而稳定训练。归一化的思想也被用于网络结构的设计，比如$Batch Normalization$。
...
1. 使用大BatchSize内存利用率提高了，大矩阵乘法的并行化效率提高，但是内存可能发生爆炸。
2. 大BatchSize跑完一次epoch(全数据集)所需的迭代次数减少，对于相同数据量的处理速度进一步加快。
3. 在一定范围内，一般来说BatchSize越大，其确定的下降方向越准，引起训练震荡越小，但是BatchSize 增大到一定程度，其确定的下降方向已经基本不再变化。
4. 并且BatchSize过大容易造成内存溢出、训练时间增加、收敛缓慢、局部最优，泛化性差等问题。

### **5.4 调节 BatchSize 对训练效果有什么影响？**

1. BatchSize太小，模型表现效果极其糟糕(error飙升)。
2. 随着BatchSize增大，处理相同数据量的速度越快，达到相同精度所需要的epoch数量越来越多。
3. 由于上述两种因素的矛盾， BatchSize增大到某个时候，达到时间上的最优。
4. 由于最终收敛精度会陷入不同的局部极值，因此BatchSize增大到某些时候，达到最终收敛精度上的最优。
...
$$
\frac{\mathrm{x}{\mathrm{i}}-\mathrm{x}{\min }}{\mathrm{x}{\max }-\mathrm{x}{\min }}
$$

标准化：

$$
\frac{\mathrm{x}_{\mathrm{i}}-\mathrm{u}}{\sigma}
$$
...
另外归一化只与数据的最大值和最小值有关，缩放比例为 $α=X{max}-X{min}$ ，而标准化的缩放比例等于标准差 $α=σ$ ，平移量等于均值 $β=μ$ ，当除了极大值极小值的数据改变时他的缩放比例和平移量也会改变。

### **6.1 为什么要归一化？**

引入归一化，是由于不同的特征之间，其量纲或者量纲单位往往不同，变化区间也处于不同的数量级，若不进行归一化，可能导致某些指标被忽略，影响到数据分析的结果。
例如影响房价的两个特征，面积和房间数量，面积有 $80, 90,100$ 等，房间数有 $1, 2 ,3$ 等，这两个指标度量方式根本不在一个数量级上。
为了消除特征之间的量纲影响，需要进行归一化处理，以解决特征指标之间的可比性，原始数据，经归一化处理后，各个指标处于同一种数量级，可以直接对比评价。

### **6.2 为什么归一化能提高求解最优解速度？**

![8944030d9cf80ef319691ed69081c05d](/assets/images/ai/computervision/deep-learning-fundamentals/8944030d9cf80ef319691ed69081c05d.png)
...
含义：经过处理的数据符合标准正态分布，即均值为 0，标准差为 1 其中 \mu为所有样本数据的均值，$\sigma$ 为所有样本数据的标准差。
...
这是一个争议比较大的方法，$LRN$ 技术主要是深度学习训练时的一种提高准确度的技术方法。其中$caffe$、$tensorflow$等里面是很常见的方法，其跟激活函数是有区别的， $LRN$ 一般是在激活、池化后进行的一种处理方法。$LRN$ 归一化技术首次在 $AlexNet$ 模型中提出这个概念。通过实验确实证明它可以提高模型的泛化能力，但是提升的很少，以至于后面不再使用，甚至有人觉得它是一个"伪命题"，因而它饱受争议，在此不做过多介绍。

详细的描述大家可以参考这篇论文：[/assets/files/ai/computervision/deep-learning-fundamentals/imagenet.pdf](/assets/files/ai/computervision/deep-learning-fundamentals/imagenet.pdf)

### **6.5 批归一化(Batch Normalization，BN)**

在之前的神经网络训练中，只是对输入层数据进行归一化处理，却没有在中间层进行归一化处理。要知道，虽然我们对输入数据进行了归一化处理，但是输入数据经过 $\sigma(WX+b)$ 这样的矩阵乘法以及非线性运算之后，其数据分布很可能被改变，而随着深度网络的多层运算之后，数据分布的变化将越来越大。如果我们能在网络的中间也进行归一化处理，是否对网络的训练起到改进作用呢？答案是肯定的。

这种在神经网络中间层也进行归一化处理，使训练效果更好的方法，就是批归一化**Batch Normalization(BN)**。

那么使用Batch Normalization到底有哪些**优点**呢？

1. 减少了人为选择参数。在某些情况下可以取消 $Dropout L2$ 正则项参数,或者采取更小的 $L2$ 正则项约束参数；
...
其中  $\epsilon$ 是为了避免分母为 0而加进去的接近于 0 的很小值。
...
> 注：上述是 BN训练时的过程，但是当在投入使用时，往往只是输入一个样本，没有所谓的均值 $\mu{\beta}$  和标准差 $\sigma$${\beta}^2$ 。此时，均值 $\mu{\beta}$ 是计算所有batch  $\mu{\beta}$ 值的平均值得到，标准差 $\sigma{\beta}^2$ 采用每个$batch$  $\sigma{\beta}^2$ 的无偏估计得到。

### **6.7 群组归一化(Group Normalization，GN )**

Group Normalization是何恺明团队2018年提出来的, $GN$优化了 $BN$ 在比较小的 mini-batch 情况下表现不太好的劣势。

Group Normalization(GN)首先将 Channels 划分为多个groups，再计算每个group内的均值和方差，以进行归一化。$GN$ 的计算与BatchSize无关，因此对于高精度图片小batchSize的情况也是非常稳定的。

### **6.8 权重归一化(Weight Normalization，WN)**

$BN/LRN/GN$ 都是在数据的层面上做的归一化，而Weight Normalization( $WN$ )是对网络权值 W 做的归一化。$WN$ 的做法是将权值向量 $w$ 在其欧氏范数和其方向上解耦成了参数向量 $v$ 和参数标量 $g$ 后使用SGD分别优化这两个参数。$WN$也是和样本量无关的，所以可以应用在batchsize 较小以及 RNN 等动态网络中；另外 BN使用的基于mini-batch的归一化统计量代替全局统计量，相当于在梯度计算中引入了噪声。而 WN 则没有这个问题，所以在生成模型，强化学习等噪声敏感的环境中 WN 的效果也要优于BN。

WN 没有额外参数，不需要额外的存储空间来保存 mini batch 的均值和方差，这样更节约显存。同时 WN 的计算效率也要优于要计算归一化统计量的 BN。
...
1. finetune的过程相当于继续训练，跟直接训练的区别是初始化方式不同。
...
权重初始化(weight initialization)又称参数初始化，在深度学习模型训练过程的本质是对 weight(即参数W)进行更新，但是在最开始训练的时候是无法更新的，这需要每个参数有相应的初始值。在进行权重初始化后，神经网络就可以对权重参数w不停地迭代更新，以达到较好的性能。
...
**把权重初始化为较小的值**，如均值为 $0$，方差为 $0.01$ 的高斯分布，随着层数的增加，输出值迅速向 0 靠拢，在后几层中，几乎所有的输出值 x都很接近0。高斯初始化，给权重较小的值，这种更新方式在小型网络中很常见，然而当网络 deep 的时候，会出现**梯度消失**的情况。

但是如果把**权重初始成一个比较大的值**，如均值为0，方差为1的高斯分布，几乎所有的值集中在 −1 或 1 附近，则会造成前向传播时，神经元要么被抑制，要么被饱和。当神经网络的层数增多时，会发现越往后面的层的激活函数($Tanh$ 在 -1 和 1附近的 gradient 都接近 0 )的输出值几乎都接近于 0。梯度更新时，梯度非常接近于 0，就会导致**梯度消失**。

### **8.3 Xavierc初始化**

也称为`Glorot初始化`，因为发明人为 Xavier Glorot 。`Xavier初始化`是 Glorot等人为了解决随机初始化的问题提出来的另一种初始化方法，他们的思想就是尽可能的让输入和输出服从相同的分布，这样就能够避免后面层的激活函数的输出值趋向于0。
...
$$
decayed\_learning\_rate =learning\_rate*decay\_rate^{\frac{globa\_step}{decay\_steps}}
$$

这种衰减方式简单直接，收敛速度快，是最常用的学习率衰减方式，如下图所示，绿色的为学习率随训练次数的指数衰减方式，红色的即为分段常数衰减，它在一定的训练区间内保持学习率不变。

![18635042df10a314c47e56c580a69415](/assets/images/ai/computervision/deep-learning-fundamentals/18635042df10a314c47e56c580a69415.png)
...
![0594ff39e1f395906a64ce80dd049102](/assets/images/ai/computervision/deep-learning-fundamentals/0594ff39e1f395906a64ce80dd049102.png)
...
$$
decay\_steps = decay\_steps * ceil (\frac {global\_step} {decay\_steps})
$$

如下图所示，红色线代表学习率降低至最低后，一直保持学习率不变进行更新，绿色线代表学习率衰减到最低后，又会再次循环往复的升高降低。

![193fa23ec6d2b66faeb640ce9ea0b8ab](/assets/images/ai/computervision/deep-learning-fundamentals/193fa23ec6d2b66faeb640ce9ea0b8ab.png)
...
![954336c18965303444340fcf2510a7ae](/assets/images/ai/computervision/deep-learning-fundamentals/954336c18965303444340fcf2510a7ae.png)
...
![162c595d368370653741776c47dcf2fd](/assets/images/ai/computervision/deep-learning-fundamentals/162c595d368370653741776c47dcf2fd.png)

下面介绍一些实际训练中具体的学习率调整方式

### **9.3.6 StepLR**

这是最简单常用的学习率调整方法，每过step_size轮，将此前的学习率乘以$\gamma$ 。

```python
scheduler=lr_scheduler.StepLR(optimizer, step_size=30, gamma=0.1)
```

![81e9882d18f2a629a7d550ef306cc1e3](/assets/images/ai/computervision/deep-learning-fundamentals/81e9882d18f2a629a7d550ef306cc1e3.png)

### **9.3.7 MultiStepLR**

`MultiStepLR`同样也是一个非常常见的学习率调整策略，它会在每个milestone时，将此前学习率乘以 \gamma 。

```python
scheduler = lr_scheduler.MultiStepLR(optimizer, milestones=[30,80], gamma=0.5)
```

![5525c93b4ccbb344765e65bcef82f7e5](/assets/images/ai/computervision/deep-learning-fundamentals/5525c93b4ccbb344765e65bcef82f7e5.png)

### **9.3.8 ExponentialLR**

`ExponentialLR`是指数型下降的学习率调节器，每一轮会将学习率乘以 $\gamma$，所以这里千万注意 $\gamma$ 不要设置的太小，不然几轮之后学习率就会降到 0。

```plain text
scheduler=lr_scheduler.ExponentialLR(optimizer, gamma=0.9)
```

![b0d37ebbe5432ef8e94e446c90afa45b](/assets/images/ai/computervision/deep-learning-fundamentals/b0d37ebbe5432ef8e94e446c90afa45b.png)

### **9.3.9 LinearLR**

`LinearLR`是线性学习率，给定起始factor和最终的factor，`LinearLR`会在中间阶段做线性插值，比如学习率为 0.1，起始factor为 1，最终的factor为 0.1，那么第 0 次迭代，学习率将为 0.1，最终轮学习率为 0.01 。下面设置的总轮数total_iters为 80,所以超过 80时，学习率恒为 0.01 。

```python
scheduler=lr_scheduler.LinearLR(optimizer,start_factor=1,end_factor=0.1,total_iters=80)
```

![f44d1b911ad0f10818ae71a9cf9b6975](/assets/images/ai/computervision/deep-learning-fundamentals/f44d1b911ad0f10818ae71a9cf9b6975.png)
...
![d8fc6a1d1c27266813b7c3ab43e7a977](/assets/images/ai/computervision/deep-learning-fundamentals/d8fc6a1d1c27266813b7c3ab43e7a977.png)

### **9.3.11 OneCycleLR**

`OneCycleLR`顾名思义就像是`CyclicLR`的一周期版本，它也有多个参数，max_lr就是最大学习率，pct_start是学习率上升部分所占比例，一开始的学习率为 $\frac {max_lr} {div_factor}$,最终的学习率为$\frac {max_lr}{final_div_factor}$，总的迭代次数为total_steps。

```plain text
scheduler=lr_scheduler.OneCycleLR(optimizer,max_lr=0.1,pct_start=0.5,total_steps=120,div_factor=10,final_div_factor=10)
```

![6df938a41f7f17e20afc317c73d5119c](/assets/images/ai/computervision/deep-learning-fundamentals/6df938a41f7f17e20afc317c73d5119c.png)
...
![7d29b4a0d48150ade98d2af55d5cfcdf](/assets/images/ai/computervision/deep-learning-fundamentals/7d29b4a0d48150ade98d2af55d5cfcdf.png)
...
![5b4d17d568a100d3b0228a1e6cb13625](/assets/images/ai/computervision/deep-learning-fundamentals/5b4d17d568a100d3b0228a1e6cb13625.png)
...
![eec5d800165659e7f90f77a21bc20f83](/assets/images/ai/computervision/deep-learning-fundamentals/eec5d800165659e7f90f77a21bc20f83.png)
...
![39a3d4062abfc398dee2533c6587b5e3](/assets/images/ai/computervision/deep-learning-fundamentals/39a3d4062abfc398dee2533c6587b5e3.png)
...
![5e8800cc4ceaf25ff2523154068f672d](/assets/images/ai/computervision/deep-learning-fundamentals/5e8800cc4ceaf25ff2523154068f672d.png)
...
![7d3b3f3b828d7317e91928def952f532](/assets/images/ai/computervision/deep-learning-fundamentals/7d3b3f3b828d7317e91928def952f532.png)
...
![4144781b071ce5fdcc6cff0c0dd4eae5](/assets/images/ai/computervision/deep-learning-fundamentals/4144781b071ce5fdcc6cff0c0dd4eae5.png)
...
![d9ad197351760f02f3b4c9cba063b6ce](/assets/images/ai/computervision/deep-learning-fundamentals/d9ad197351760f02f3b4c9cba063b6ce.png)
...
$$
\min _{\theta} \frac {1} {2m} \sum_{i=1}^{m}\left(h_{\theta}\left(x^{(i)}\right)-y^{(i)}\right)^{2}+1000θ_3^2+1000θ_4^2
$$
...
其中 $\lambda$ 叫做正则化参数， $\lambda$ 越大则惩罚力度也越大，但 $\lambda$ 并不是越大越好当 $\lambda$ 太大时就会造成拟合函数中的参数太小以至于拟合函数就等于 $θ_0$ 变成一条直线，造成欠拟合。
...
$$
\|\vec{x}\|_{2}=\sqrt{\sum_{i=1}^{N}\left|x_{i}\right|^{2}}
$$
...
$$
L_{p}=\|\vec{x}\|_{p}=\sqrt[p]{\sum_{i=1}^{N}\left|x_{i}\right|^{p}}
$$

### **10.3 Dropout正则化**

![b2ea21d008e76bafc942093813a99b99](/assets/images/ai/computervision/deep-learning-fundamentals/b2ea21d008e76bafc942093813a99b99.png)
...
直观上理解：不要依赖于任何一个特征，因为该单元的输入可能随时被清除，因此该单元通过这种方式传播下去，并为单元的四个输入增加一点权重，通过传播所有权重， $Dropout$ 将产生收缩权重的平方范数的效果，和之前讲的 L2 正则化类似；实施 $Dropout$ 的结果实它会压缩权重，并完成一些预防过拟合的外层正则化； L2对不同权重的衰减是不同的，它取决于激活函数倍增的大小。
...
[8] [深度学习超参数介绍及调参](https://blog.csdn.net/xu_fu_yong/article/details/96102999?ops_request_misc=%257B%2522request%255Fid%2522%253A%2522166458703116782414988012%2522%252C%2522scm%2522%253A%252220140713.130102334..%2522%257D&request_id=166458703116782414988012&biz_id=0&utm_medium=distribute.pc_search_result.none-task-blog-2~all~top_positive~default-1-96102999-null-null.142%5Ev51%5Econtrol%2C201%5Ev3%5Econtrol_2&utm_term=%E8%B6%85%E5%8F%82%E6%95%B0&spm=1018.2226.3001.4187)

[9] [https://www.zhihu.com/question/67366051/answer/262087707](https://www.zhihu.com/question/67366051/answer/262087707)

[10] [https://blog.csdn.net/weiman1/article/details/125647517?spm=1001.2014.3001.5506](https://blog.csdn.net/weiman1/article/details/125647517?spm=1001.2014.3001.5506)
