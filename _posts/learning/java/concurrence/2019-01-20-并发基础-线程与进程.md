...
date: 2019-01-20
slug: java-concurrency-thread-basics
categories:
- Java
tags:
- Java
...
type: note
status: draft
created: 2019-01-20 17:14
updated: 2019-01-20 17:14
week: 2025-W48
---
...
![JVM进程未结束示意](/assets/images/learning/java/concurrence/java-concurrency-thread-basics/bf6d78b273b6f4988dd9855fc1026631.png)
...
CAS（Compare And Swap）是无锁并发核心手段。典型风险是 ABA 问题：值从 A 变 B 又回 A，CAS 无法察觉"曾变化过"。
常见处理：`AtomicStampedReference`（版本号）。

### 指令重排序

JMM 允许编译器/处理器在不破坏单线程语义前提下重排序。
多线程下需要通过 `volatile`、锁、happens-before 规则建立正确的执行约束。
