---
layout: post
title: 死磕Java并发：J.U.C之并发工具类：CyclicBarrier
...
- generation 变量，表示 CyclicBarrier 的更新换代。详细解析，见 [「2.4 Generation」](https://www.iocoder.cn/JUC/sike/CyclicBarrier/#) 。
...
- 返回值为进入屏障时，剩余拦截线程的**剩余需要**数量。英文注释如下：the arrival index of the current thread, where index {@code getParties() - 1} indicates the first to arrive and zero indicates the last to arrive
...
- 如果一个线程处于等待状态时，如果其他线程调用 #reset() 方法。详细解析，见 [「2.7 reset」](https://www.iocoder.cn/JUC/sike/CyclicBarrier/#) 。
- 调用的 barrier 原本就是被损坏的，则抛出 BrokenBarrierException 异常。
- 任何线程在等待时被中断了，则其他所有线程都将抛出 BrokenBarrierException 异常，并将 barrier 置于**损坏**状态。详细解析，见 [「2.6 breakBarrier」](https://www.iocoder.cn/JUC/sike/CyclicBarrier/#) 。
