...
![本地暴露顺序图](/assets/images/learning/dubbo/dubbo-service-expose-injvm/460c075ce0b16a112736251fa3a121d2.png)
...
- 第 8 至 11 行：循环 `protocols`，调用 `#doExportUrlsFor1Protocol(protocolConfig, registryURLs)` 方法，使用对应的协议注册中心分组因为仅仅用于 JVM 内部本地调用，内存中已经有相关信息，逐个向暴露服务。在这个方法中，包含了本地和远程两种暴露方式。在下文中，我们会看到，本地暴露不会向注册中心注册服务。
...
- 第 46 行：调用 [UrlUtils#parseURLs(address, map)](https://github.com/YunaiV/dubbo/blob/8de6d56d06965a38712c46a0220f4e59213db72f/dubbo-common/src/main/java/com/alibaba/dubbo/common/utils/UrlUtils.java#L147-L174) 方法，解析 `address`，创建 Dubbo URL 数组（数组大小可以为一）。
    - 已经添加了代码注释，胖友点击链接查看。
    - `address` **注意**可以使用 `"|"` 或者 `";"` 作为分隔符，设置多个注册中心**分组**，一个注册中心集群是一个分组，而不是多个。
    - 这个方法从名字上很难看出具体的含义，看下图的**注释**，相信胖友能理解。

![UrlUtils#parseURLs(address, map) 注释](/assets/images/learning/dubbo/dubbo-service-expose-injvm/23dbe8389dd09ec1ae510ae073e721c1.png)
...
![数据流向](/assets/images/learning/dubbo/dubbo-service-expose-injvm/4f1a941b7abd39f1421cbe5c04eade9b.png)
...
private T ref;
/**
 * 服务配置暴露的 Exporter。URL ：Exporter 不一定是 1：1 的关系。
...
- 第 18 行：调用 `Protocol#export(invoker)` 方法，暴露服务。
    - 此处 Dubbo SPI **自适应**的好处就出来了，可以**自动**根据 URL 参数，获得对应的拓展实现。例如，`invoker` 传入后，根据 `invoker.url` 自动获得对应 Protocol 拓展实现为 InjvmProtocol。
    - 实际上，Protocol 有两个 Wrapper 拓展实现类：ProtocolFilterWrapper、ProtocolListenerWrapper。所以，**Protocol$Adaptive => ProtocolFilterWrapper => ProtocolListenerWrapper => InjvmProtocol** #export(…) 方法的调用顺序是：
...
![Protocol 类图](/assets/images/learning/dubbo/dubbo-service-expose-injvm/7742051807eebec681288e420bd64813.png)

## 3.1 Protocol$Adaptive

代码类似下图，笔者偷懒，就不去重新生成啦。

![自适应拓展的代码实现的字符串生成例子](/assets/images/learning/dubbo/dubbo-service-expose-injvm/c73cb291227ae3a3c7e5d0b0f8e6f5b6.png)
...
- `key` 属性，获得 URL 参数名。
    ...
    - 以 ServiceConfig 举例子，例如 `**service.filter=demo**`，这是笔者配置自定义的 DemoFilter 过滤器。
...
![Exporter 类图](/assets/images/learning/dubbo/dubbo-service-expose-injvm/c145ab1413aef756870a00567918e13f.png)
...
![ExporterListener 子类](/assets/images/learning/dubbo/dubbo-service-expose-injvm/9ec4e9d85430acc27261d22ade770c65.png)
...
![知识星球](/assets/images/learning/dubbo/dubbo-service-expose-injvm/96ca62e95e06bbe2fa74153d2158bc13.png)
