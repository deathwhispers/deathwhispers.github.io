...
- 通过读取**类大体**[《深入理解RPC之序列化篇 – Kryo》](https://www.cnkirito.moe/2017/11/28/rpc-serialize-1/)[「三种读写方式」](http://svip.iocoder.cn/Dubbo/serialize-3-kryo/#)，在根据类解析具体对象，字节内容"是 [ Class, 对象二进制数据 ] 。在对这块解析的相当不错。
...
- 通过写入**类大体**[《深入理解RPC之序列化篇 – Kryo》](https://www.cnkirito.moe/2017/11/28/rpc-serialize-1/)[「三种读写方式」](http://svip.iocoder.cn/Dubbo/serialize-3-kryo/#)+ 具体对象，字节内容"是 [ Class, 对象二进制数据 ] 。在对这块解析的相当不错。
...
- 第 6 至 12 行：Kryo 不支持不包含**空构造方法Java 原生序列化**的类的序列化，因此，此时使用 Kryo 封装实现类com.esotericsoftware.kryo.serializers.JavaSerializer。
...
- registrations**静态注册**属性，需要的类的集合。通过#registerClass(Class)方法，可以添加，代码如下：
...
- registrationRequired**注册关闭一般关闭注册行为**属性，是否开启行为，默认。Kryo 支持对注册行为，如kryo.register(SomeClazz.class);，这会赋予该 Class 一个从 0 开始的编号，但 Kryo 使用注册行为最大的问题在于，其不保证同一个 Class 每一次注册的号码相同，这与注册的顺序有关，也就意味着在不同的机器、同一个机器重启前后都有可能拥有不同的编号，这会导致序列化产生问题，所以在分布式项目中，。
