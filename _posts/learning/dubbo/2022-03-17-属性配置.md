...
- **外部化配置** [《Dubbo 外部化配置（Externalized Configuration）》](https://github.com/mercyblitz/blogs/blob/master/java/dubbo/Dubbo-Externalized-Configuration.md)，能够解决上述的问题，感兴趣的胖友可以自己看下。当然，这块内容后面分享，不在本文的范畴。
...
![读取调用](/assets/images/learning/dubbo/dubbo-configuration-properties/346401673e36b142140de8fbda32259a.png)
    ...
    for (Method method : methods) {
        try {
            ...
            if (name.length() > 3 && name.startsWith("set") && Modifier.isPublic(method.getModifiers())
                    && method.getParameterTypes().length == 1 && isPrimitive(method.getParameterTypes()[0])) { // 方法是 public 的 setting 方法，且方法的唯一参数是基本数据类型
...
- 第 5 行：获得配置项**前缀** [#getTagName(Class<?>)](https://github.com/YunaiV/dubbo/blob/d3c3975f320c78452f96098b04441fed4c00ab70/dubbo-config/dubbo-config-api/src/main/java/com/alibaba/dubbo/config/AbstractConfig.java#L214-L230)。此处的 `getTagName` 方法，使用配置类的类名，获得对应的属性标签。该方法代码如下：
...
- 第 10 至 11 行：public && setting 方法 && **唯一**参数为基本类型。
    - 其中**唯一**参数为基本类型，决定了一个配置对象无法设置另外一个配置对象为属性，即**属性配置**不支持多注册中心，多协议等情况。例如，ServiceConfig 无法通过属性配置设置多个 ProtocolConfig 对象。
    ...
    FROM [《Dubbo 用户指南 —— 属性配置》](http://dubbo.apache.org/zh-cn/docs/user/configuration/properties.html)
    在 Dubbo 官方用户手册的 "属性配置" 章节中，dubbo.properties 配置属性能够映射到 ApplicationConfig、ProtocolConfig 以及 RegistryConfig 的字段。从某种意义上来说，dubbo.properties 也是 Dubbo 的外部化配置。
- 第 13 行：获得属性名。例如，`ApplicationConfig#setName(…)` 方法，对应的属性名为 `"name"`。
- 读取的**覆盖策略**如下：
![覆盖策略](/assets/images/learning/dubbo/dubbo-configuration-properties/29fabfc0e001f935eee101bb81415536.png)
- 第 15 至 31 行：优先从【**启动参数变量**】获取配置项的值。
    - 有**两种**情况，胖友细看下注释。
- 第 33 至 45 行：因为 XML配置 的优先级**大于** properties配置，因此需要使用 getting 方法，判断配置对象已经拥有该配置项的值。如果有，则不从 properties配置 读取对应的值。
