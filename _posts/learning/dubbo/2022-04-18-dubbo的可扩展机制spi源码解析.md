...
![Dubbo SPI 架构图](/assets/images/learning/dubbo/dubbo-spi-source-code/076926bd5cc65738b333670db3911b88.png)
...
6. `getAdaptiveExtension()`：表示获取一个自适应的扩展点实例
7. `getActivateExtension(URL url, String[] values, String group)`：表示一个可以被url激活的扩展点实例，后文详细解释
...
![getExtension流程](/assets/images/learning/dubbo/dubbo-spi-source-code/f2a0e5b1e25891faee724cf098e66909.png)
...
8. 根据name找到对应的扩展点实现类
9. 根据实现类生成一个实例，把**实现类和对应生成的实例**进行缓存
10. 对生成出来的实例进行依赖注入（给实例的属性进行赋值）
11. 对依赖注入后的实例进行AOP（Wrapper）,把当前接口类的所有的Wrapper全部一层一层包裹在实例对象上，没包裹个Wrapper后，也会对Wrapper对象进行依赖注入
12. 返回最终的Wrapper对象

![createExtension流程](/assets/images/learning/dubbo/dubbo-spi-source-code/b9d111aa035d8c90e1082708aa5abc7f.png)
...
13. 根据接口的全限定名去**META-INF/dubbo/internal/**目录下寻找对应的文件，调用loadResource方法进行加载
14. 根据接口的全限定名去**META-INF/dubbo/**目录下寻找对应的文件，调用loadResource方法进行加载
15. 根据接口的全限定名去**META-INF/services/**目录下寻找对应的文件，调用loadResource方法进行加载
...
16. 当前扩展点实现类上是否存在@Adaptive注解，如果存在则把该类认为是当前接口的默认自适应类（接口代理类），并把该类存到cachedAdaptiveClass属性上。
...
19. 如果有多个name，则判断一下当前扩展点实现类上是否存在@Activate注解，如果存在，则把该类添加到cachedActivates中，cachedWrapperClasses是一个map
...
24. 调用`objectFactory.getExtension(pt, property)`得到一个对象，这里就会从Spring容器或通过DubboSpi机制得到一个对象，比较特殊的是，如果是通过Dubbo Spi机制得到的对象，是pt这个类型的一个自适应对象(代理对象)。
