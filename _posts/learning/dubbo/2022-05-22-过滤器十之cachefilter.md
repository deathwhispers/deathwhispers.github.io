...
![类图](/assets/images/learning/dubbo/dubbo-filter-cache/7e84fb4f8d34002cbd3f0215e302dc93.png)
...
- 第 18 行：判断方法**开启部分** Cache 功能。因为，一个服务里，可能只有方法开启了 Cache 功能。
...
- 第 31 至 34 行：若**非异常正常的**，调用 Cache#put(key, value) 方法，缓存结果。
- 第 35 行：返回调用结果。
- 第 39 行：若**不使用直接** Cache 功能，调用 Invoker#invoke(invocation) 方法，服务调用。
...
- `@SPI("lru")` **拓展点**注解，Dubbo SPI，默认为 `"lru"`。
