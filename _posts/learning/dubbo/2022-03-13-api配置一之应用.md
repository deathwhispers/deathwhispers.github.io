...
![dubbo-config-api 项目结构](/assets/images/learning/dubbo/dubbo-api-configuration-application/371342e80cf080538309be93386c3f20.png)

一脸懵逼，好多啊。下面我们来整理下**配置之间的关系**，如下图所示：

FROM [《Dubbo 用户指南 —— XML 配置》](http://dubbo.apache.org/zh-cn/docs/user/configuration/xml.html)

![配置之间的关系](/assets/images/learning/dubbo/dubbo-api-configuration-application/01ee36c503e6811d484719e7145f5505.png)
...
![配置类关系](/assets/images/learning/dubbo/dubbo-api-configuration-application/e7430b4467cbda65211cd243f6849d4a.png)
...
- 第 54 至 57 行：当方法为 `#getParameters()` 时，[例如](https://github.com/YunaiV/dubbo/blob/9d38b6f9f95798755141d6140e311e8fd51fecc1/dubbo-config/dubbo-config-api/src/main/java/com/alibaba/dubbo/config/ApplicationConfig.java#L253-L255)。
    ...
    - 因此，通过 `#getParameters()` 对应的属性，可以**动态设置配置项，拓展出非 Dubbo 内置好的逻辑**。
...
- 不同于 [#appendParameters(parameters, config, prefix)](https://github.com/YunaiV/dubbo/blob/9d38b6f9f95798755141d6140e311e8fd51fecc1/dubbo-config/dubbo-config-api/src/main/java/com/alibaba/dubbo/config/AbstractConfig.java#L326-L363) 方法，主要用于[《Dubbo 用户指南 —— 事件通知》](http://dubbo.apache.org/zh-cn/docs/user/demos/events-notify.html)，注解 `@Parameter(attribute = true)` 的属性如下图：

![@Parameter(attribute = true)](/assets/images/learning/dubbo/dubbo-api-configuration-application/2601b172743f7da6d1b26a6e5a73bb1d.png)

- 第 9 至 13 行：方法为获得**基本类型** + `public` 的 getting 方法。
- 第 14 至 16 行：**需要** `@Parameter.attribute = true` 的配置项。
- 第 17 至 24 行：获得配置项**名**。
- 第 26 至 30 行：获得配置项**值**。
...
![@Parameter 使用场景](/assets/images/learning/dubbo/dubbo-api-configuration-application/df06be16802cdf8e582df6f5706900cb.png)
