...
type: note
status: draft
created: 2020-09-16 09:00
updated: 2020-09-16 18:00
---
...
1. 转换 beanName 。因为我们调用
#getBean(…)
方法传入的
name
并不一定就是 beanName，可以传入 aliasName，FactoryBean，所以这里需要进行简单的转换过程。
2. 尝试从缓存中加载单例 bean 。
3. bean 的实例化。
4. 原型模式的依赖检查。因为 Spring 只会解决单例模式的循环依赖，对于原型模式的循环依赖都是直接抛出 BeanCurrentlyInCreationException 异常。
5. 尝试从 parentBeanFactory 获取 bean 实例。如果
parentBeanFactory != null && !containsBeanDefinition(beanName)
则尝试从 parentBeanFactory 中获取 bean 实例对象，因为
!containsBeanDefinition(beanName)
就意味着定义的 xml 文件中没有 beanName 相应的配置，这个时候就只能从
parentBeanFactory
中获取。
6. 获取 RootBeanDefinition，并对其进行合并检查。从缓存中获取已经解析的 RootBeanDefinition 。同时，如果父类不为
null
的话，则会合并父类的属性。
...
不为
null
，则需要检测所得到 bean 的类型是否与该
requiredType
一致。如果不一致则尝试转换，当然也要能够转换成功，否则抛出 BeanNotOfRequiredTypeException 异常。
...
- 该方法是定义在 AbstractBeanFactory 中的**抽象**
方法，其含义是根据给定的 BeanDefinition 和
args
...
- <1>
处，如果是单例模式，则清除
factoryBeanInstanceCache 缓存
，同时返回 BeanWrapper 实例对象，当然如果存在。
- <2>
处，如果缓存中没有 BeanWrapper 或者不是单例模式，则调用
#createBeanInstance(…)
方法，实例化 bean，主要是将 BeanDefinition 转换为 BeanWrapper 。
- <3>
...
13. Supplier 回调
14. #instantiateUsingFactoryMethod(…)
方法，工厂方法初始化
15. #autowireConstructor(…)
方法，构造函数自动注入初始化
16. #instantiateBean(…)
方法，默认构造函数注入
...
- <1.1>**保证全局唯一**
首先，获取锁。其实我们在前面篇幅中发现了大量的同步锁，锁住的对象都是
...
方法，从 FactoryBean 获取对象，其实内部就是调用
...
17. [【死磕 Spring】—— IoC 之加载 bean：开启 bean 的加载](http://svip.iocoder.cn/Spring/IoC-get-Bean-begin)
18. [【死磕 Spring】—— IoC 之加载 bean：从单例缓存中获取单例 bean](http://svip.iocoder.cn/Spring/IoC-get-Bean-getSingleton)
19. [【死磕 Spring】—— IoC 之加载 bean：parentBeanFactory 与依赖处理](http://svip.iocoder.cn/Spring/IoC-get-Bean-parentBeanFactory-and-depend-on)
20. [【死磕 Spring】—— IoC 之加载 bean：分析各 scope 的 bean 创建](http://svip.iocoder.cn/Spring/IoC-get-Bean-different-scope)
21. [【死磕 Spring】—— IoC 之加载 bean：创建 bean（一）之主流程](http://svip.iocoder.cn/Spring/IoC-get-Bean-createBean-1)
22. [【死磕 Spring】—— IoC 之加载 bean：创建 bean（二）之实例化 Bean 对象(1)](http://svip.iocoder.cn/Spring/IoC-get-Bean-createBean-2)
23. [【死磕 Spring】—— IoC 之加载 bean：创建 bean（三）之实例化 Bean 对象(2)](http://svip.iocoder.cn/Spring/IoC-get-Bean-createBean-3)
24. [【死磕 Spring】—— IoC 之加载 bean：创建 bean（四）之属性填充](http://svip.iocoder.cn/Spring/IoC-get-Bean-createBean-4)
25. [【死磕 Spring】—— IoC 之加载 bean：创建 bean（五）之循环依赖处理](http://svip.iocoder.cn/Spring/IoC-get-Bean-createBean-5)
26. [【死磕 Spring】—— IoC 之加载 bean：创建 bean（六）之初始化 Bean 对象](http://svip.iocoder.cn/Spring/IoC-get-Bean-createBean-6)
