---
layout: post
title: IOC-深入分析PropertyPlaceholderConfigurer
author: deathwhispers
date: 2020-11-23
slug: ioc-deep-dive-into-property-placeholder-configurer
categories:
- Spring
tags:
- Spring
...
type: note
status: draft
created: 2020-11-23 09:00
updated: 2020-11-23 18:00
---
...
![5e0412b774d61c11e972a7bb9a1da16b](/assets/images/learning/spring/springsourcecode/ioc-deep-dive-into-property-placeholder-configurer/5e0412b774d61c11e972a7bb9a1da16b.png)
...
- 在构造 String 值解析器 StringValueResolver 时，将已经解析的 Properties 实例对象封装在 PlaceholderResolver 实例
resolver
中。PlaceholderResolver 是一个用于解析字符串中包含占位符的替换值的策略接口，该接口有一个
#resolvePlaceholder(String strVa)
方法，用于返回占位符的替换值。
- 还有一个 PropertyPlaceholderHelper 工具
helper
，从名字上面看应该是进行替换的工具类。
...
```plain text
java // BeanDefinitionVisitor.java  protected void visitPropertyValues(MutablePropertyValues pvs) {     PropertyValue[] pvArray = pvs.getPropertyValues();     // 遍历 PropertyValue 数组     for (PropertyValue pv : pvArray) {         // 解析真值         Object newVal = resolveValue(pv.getValue());         if (!ObjectUtils.nullSafeEquals(newVal, pv.getValue())) {             // 设置到 PropertyValue 中             pvs.add(pv.getName(), newVal);         }     } }
```
...
- helper
为 PropertyPlaceholderHelper 实例对象，而 PropertyPlaceholderHelper 则是处理应用程序中包含占位符的字符串工具类。在构造
helper
实例对象时需要传入了几个参数：
placeholderPrefix
、
...
- 调用**这个方法是这篇博客最核心的地方**
#parseStringValue(String value, PlaceholderResolver placeholderResolver, Set visitedPlaceholders)
方法，
，
${}
占位符的替换。代码如下：
