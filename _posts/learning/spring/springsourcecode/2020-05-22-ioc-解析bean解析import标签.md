...
摘要: 原创出处 [http://cmsblogs.com/?p=2724](http://cmsblogs.com/?p=2724) 「小明哥」，谢谢！
...
```java
<?xml version="1.0" encoding="UTF-8"?><beans xmlns="http://www.springframework.org/schema/beans"xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"xsi:schemaLocation="http://www.springframework.org/schema/beanshttp://www.springframework.org/schema/beans/spring-beans.xsd">    <import resource="spring-student.xml"/>    <import resource="spring-student-dtd.xml"/></beans>
```
...
“${user.dir}”
。
- <3>[「2.1 判断路径」](http://svip.iocoder.cn/Spring/IoC-parse-BeanDefinitions-for-import/#)
处，判断资源路径
location
是绝对路径还是相对路径。详细解析，见
。
    - <4>[「2.2 处理绝对路径」](http://svip.iocoder.cn/Spring/IoC-parse-BeanDefinitions-for-import/#)
处，如果是绝对路径，则调递归调用 Bean 的解析过程，进行另一次的解析。详细解析，见
。
    - <5>[「2.3 处理相对路径」](http://svip.iocoder.cn/Spring/IoC-parse-BeanDefinitions-for-import/#)
处，如果是相对路径，则先计算出绝对路径得到 Resource，然后进行解析。详细解析，见
。
- <6>
处，通知监听器，完成解析。
...
方法，进行 BeanDefinition 加载。
- 否则，构造一个绝对 **与绝对路径过程一样**
location
( 即
StringUtils.applyRelativePath(baseLocation, location)
处的代码)，并调用
#loadBeanDefinitions(String location, Set actualResources)
方法，
。
