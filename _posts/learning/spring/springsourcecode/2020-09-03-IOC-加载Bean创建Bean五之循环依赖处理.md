---
layout: post
title: IOC-加载Bean：创建Bean（五）之循环依赖处理
slug: ioc-loop-dependency-resolution
type:
- note
date: 2020-09-03
status: draft
...
created: 2020-09-03 09:00
updated: 2020-09-03 18:00
categories:
- Learning
- Spring
---
...
- #isSingletonCurrentlyInCreation(String beanName)**因为 Spring 解决 singleton bean 的核心就在于提前曝光 bean**
方法：判断当前 singleton bean 是否处于创建中。bean 处于创建中，也就是说 bean 在初始化但是没有完成初始化，有一个这样的过程其实和 Spring 解决 bean 循环依赖的理念相辅相成。
。
- allowEarlyReference**原因就在于 singletonFactories 才是 Spring 解决 singleton bean 的诀窍所在**
变量：从字面意思上面理解就是允许提前拿到引用。其实真正的意思是，是否允许从
singletonFactories
缓存中通过
#getObject()
方法，拿到对象。为什么会有这样一个字段呢？
，这个我们后续分析。
...
- 另外，这里在推荐一篇[《Spring循环依赖三级缓存是否可以减少为二级缓存？》](https://segmentfault.com/a/1190000023647227)
文章，解释的也非常不错。
...
singletonFactories
缓存中）。
- 这样，一旦下一个 bean 创建的时候需要依赖 bean ，则直接使用 ObjectFactory 的 [「2.1 getSingleton」](http://svip.iocoder.cn/Spring/IoC-get-Bean-createBean-5/#)
#getObject()
方法来获取了，也就是
小结中的方法中的代码片段了。
...
方法来拿到 A 对象，C 拿到 A 对象后顺利完成初始化，然后将自己添加到一级缓存中
