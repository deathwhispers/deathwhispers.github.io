---
layout: post
title: IOC-加载Bean：创建Bean（四）之属性填充
author: deathwhispers
date: 2020-08-28
slug: ioc-populate-bean
categories:
- Spring
tags:
- Spring
...
type: note
status: draft
created: 2020-08-28 09:00
updated: 2020-08-28 18:00
---
...
- <1>**此过程可以控制 Spring 是否继续进行属性填充**
，根据
hasInstantiationAwareBeanPostProcessors
属性来判断，是否需要在注入属性之前给 InstantiationAwareBeanPostProcessors 最后一次改变 bean 的机会。
。
...
- 其实主要过程和根据名称自动注入**差不多比较复杂**
，都是找到需要依赖注入的属性，然后通过迭代的方式寻找所匹配的 bean，最后调用
#registerDependentBean(…)
方法，来注册依赖。不过相对于
#autowireByName(…)
方法而言，根据类型寻找相匹配的 bean 过程
。
...
- 属性值类型**不需要**
转换时，不需要解析属性值，直接准备进行依赖注入。
- 属性值**需要**
进行类型转换时，如对其他对象的引用等，首先需要解析属性值，然后对解析后的属性值进行依赖注入。
