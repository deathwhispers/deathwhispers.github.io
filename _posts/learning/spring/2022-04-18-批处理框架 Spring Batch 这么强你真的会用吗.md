...
3. **数据读取器：** 原先使用的是 `FlatFileItemReader`，我们现在改为使用 `MyBatisCursorItemReader`
4. **数据处理器：** 新的场景，业务为了好扩展，所以我们处理器最好也新建一个
5. **数据输出器：** 新的场景，业务为了好扩展，所以我们数据输出器最好也新建一个
6. **step的绑定设置：** 新的场景，业务为了好扩展，所以我们step最好也新建一个
7. **Job：** 当然是要重新写一个了
