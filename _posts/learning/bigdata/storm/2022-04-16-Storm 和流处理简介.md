...
layout: post
title: Storm 和流处理简介
slug: storm-and-stream-processing-introduction
type:
- note
date: 2022-04-16
status: draft
...
![97be2fa2f5183fc8eaab940195fbc0f0](/assets/images/learning/bigdata/storm/storm-and-stream-processing-introduction/97be2fa2f5183fc8eaab940195fbc0f0.png)
...
| 消息投递 | At Most OnceAt Least Once | At Most OnceAt Least Once**Exactly Once** |
...
![697c05f87d458d5da42a37b9dad86769](/assets/images/learning/bigdata/storm/storm-and-stream-processing-introduction/697c05f87d458d5da42a37b9dad86769.png)
...
![a665b7f13976197c17cbaad6d9645295](/assets/images/learning/bigdata/storm/storm-and-stream-processing-introduction/a665b7f13976197c17cbaad6d9645295.png)
...
- **可以处理更大的数据量**
：直接处理数据流，并且只保留数据中有意义的子集，然后将其传送到下一个处理单元，通过逐级过滤数据，从而降低实际需要处理的数据量；
- **更贴近现实的数据模型**
：在实际的环境中，一切数据都是持续变化的，想要通过历史数据推断未来的趋势，必须保证数据的不断输入和模型的持续修正，典型的就是金融市场、股票市场，流处理能更好地处理这些场景下对数据连续性和及时性的需求；
- **分散和分离基础设施**
：流式处理减少了对大型数据库的需求。每个流处理程序通过流处理框架维护了自己的数据和状态，这使其更适合于当下最流行的微服务架构。
