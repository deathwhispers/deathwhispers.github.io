...
layout: post
title: Storm核心概念详解
slug: storm-core-concepts-explanation
type:
  - note
date: 2022-04-18
status: draft
...
![1c2f6868e09532a944b117e08d85293c](/assets/images/learning/bigdata/storm/storm-core-concepts-explanation/1c2f6868e09532a944b117e08d85293c.png)
...
![e13a7c17c3d95ac8d3ac79d131518b34](/assets/images/learning/bigdata/storm/storm-core-concepts-explanation/e13a7c17c3d95ac8d3ac79d131518b34.png)
...
![141acb392c30e435ab6eff97162d731f](/assets/images/learning/bigdata/storm/storm-core-concepts-explanation/141acb392c30e435ab6eff97162d731f.png)

### 2.1 Nimbus进程

也叫做 Master Node，是 Storm 集群工作的全局指挥官。主要功能如下：

9. 通过 Thrift 接口，监听并接收 Client 提交的 Topology；
10. 根据集群 Workers 的资源情况，将 Client 提交的 Topology 进行任务分配，分配结果写入 Zookeeper;
11. 通过 Thrift 接口，监听 Supervisor 的下载 Topology 代码的请求，并提供下载 ;
12. 通过 Thrift 接口，监听 UI 对统计信息的读取，从 Zookeeper 上读取统计信息，返回给 UI;
13. 若进程退出后，立即在本机重启，则不影响集群运行。

### 2.2 Supervisor进程

也叫做 Worker Node , 是 Storm 集群的资源管理者，按需启动 Worker 进程。主要功能如下：

14. 定时从 Zookeeper 检查是否有新 Topology 代码未下载到本地 ，并定时删除旧 Topology 代码 ;
15. 根据 Nimbus 的任务分配计划，在本机按需启动 1 个或多个 Worker 进程，并监控所有的 Worker 进程的情况；
16. 若进程退出，立即在本机重启，则不影响集群运行。
...
17. 根据 Zookeeper 上分配的 Task，在本进程中启动 1 个或多个 Executor 线程，将构造好的 Task 实例交给 Executor 去运行；
18. 向 Zookeeper 写入心跳 ；
19. 维持传输队列，发送 Tuple 到其他的 Worker ；
20. 若进程退出，立即在本机重启，则不影响集群运行。
...
21. 执行 1 个或多个 Task；
22. 执行 Acker 机制，负责发送 Task 处理状态给对应 Spout 所在的 worker。

### 2.6 并行度

![31b024ee6d29550fd1c6dc4e186e329a](/assets/images/learning/bigdata/storm/storm-core-concepts-explanation/31b024ee6d29550fd1c6dc4e186e329a.png)
...
23. [storm documentation -> Concepts](http://storm.apache.org/releases/1.2.2/Concepts.html)
24. [Internal Working of Apache Storm](https://www.spritle.com/blogs/2016/04/04/apache-storm/)
25. [Understanding the Parallelism of a Storm Topology](http://storm.apache.org/releases/1.2.2/Understanding-the-parallelism-of-a-Storm-topology.html)
26. [Storm nimbus 单节点宕机的处理](https://blog.csdn.net/daiyutage/article/details/52049519)