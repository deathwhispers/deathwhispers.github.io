...
date: 2023-03-02
slug: flink-core-concepts-overview
categories:
- BigData
tags:
- Flink
...
![有界数据流和无界数据流](/assets/images/learning/bigdata/flink/flink-core-concepts-overview/5d5f888831c89e9960eb384b295d308b.png)

Spark Streaming 数据流的拆分：

![Spark Streaming数据流拆分](/assets/images/learning/bigdata/flink/flink-core-concepts-overview/97be2fa2f5183fc8eaab940195fbc0f0.png)

## 二、Flink 核心架构

Flink 采用分层的架构设计，从而保证各层在功能和职责上的清晰。如下图所示，由上而下分别是 API & Libraries 层、Runtime 核心层以及物理部署层：

![Flink核心架构](/assets/images/learning/bigdata/flink/flink-core-concepts-overview/0fd8e56c07a65ada9dabcf9448accd42.png)
...
![Flink分层API](/assets/images/learning/bigdata/flink/flink-core-concepts-overview/6ed3ac2402a5cf223f8c0a1124eab02c.png)
...
- **JobManagers***mastersleaderstandby*
(也称为
) ：JobManagers 接收由 Dispatcher 传递过来的执行程序，该执行程序包含了作业图 (JobGraph)，逻辑数据流图 (logical dataflow graph) 及其所有的 classes 文件以及第三方类库 (libraries) 等等 。紧接着 JobManagers 会将 JobGraph 转换为执行图 (ExecutionGraph)，然后向 ResourceManager 申请资源来执行该任务，一旦申请到资源，就将执行图分发给对应的 TaskManagers 。因此每个作业 (Job) 至少有一个 JobManager；高可用部署下可以有多个 JobManagers，其中一个作为
，其余的则处于
状态。
- **TaskManagers***workers*
(也称为
) : TaskManagers 负责实际的子任务 (subtasks) 的执行，每个 TaskManagers 都拥有一定数量的 slots。Slot 是一组固定大小的资源的合集 (如计算能力，存储空间)。TaskManagers 启动后，会将其所拥有的 slots 注册到 ResourceManager 上，由 ResourceManager 进行统一管理。
- **Dispatcher**
：负责接收客户端提交的执行程序，并传递给 JobManager 。除此之外，它还提供了一个 WEB UI 界面，用于监控作业的执行情况。
- **ResourceManager**
...
![Flink核心组件](/assets/images/learning/bigdata/flink/flink-core-concepts-overview/c75b4c480d2c68580e3af3ae9b123414.png)
...
![Task与SubTask](/assets/images/learning/bigdata/flink/flink-core-concepts-overview/4ec4a546065fff24a58ec77b403efed5.png)
...
![SubTask与Slots对应](/assets/images/learning/bigdata/flink/flink-core-concepts-overview/87925cae4bbd1f49b15e49668ff8084f.png)

这时每个 SubTask 线程运行在一个独立的 TaskSlot， 它们共享所属的 TaskManager 进程的TCP 连接（通过多路复用技术）和心跳信息 (heartbeat messages)，从而可以降低整体的性能开销。此时看似是最好的情况，但是每个操作需要的资源都是不尽相同的，这里假设该作业 keyBy 操作所需资源的数量比 Sink 多很多 ，那么此时 Sink 所在 Slot 的资源就没有得到有效的利用。

基于这个原因，Flink 允许多个 subtasks 共享 slots，即使它们是不同 tasks 的 subtasks，但只要它们来自同一个 Job 就可以。假设上面 souce & map 和 keyBy 的并行度调整为 6，而 Slot 的数量不变，此时情况如下：

![Slot共享](/assets/images/learning/bigdata/flink/flink-core-concepts-overview/42a2761f53dc1ca59ff717e210de4d7a.png)
...
![Slot与并行度](/assets/images/learning/bigdata/flink/flink-core-concepts-overview/ab89e0ebac16af7ab8e14adf8d8e3bd6.png)
...
![组件通讯](/assets/images/learning/bigdata/flink/flink-core-concepts-overview/fe3a714e647a79a121b316b5d87b677f.png)
