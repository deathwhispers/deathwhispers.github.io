...
date: 2023-03-05
slug: flink-development-environment-setup
categories:
- BigData
tags:
- Flink
...
![Scala插件安装](/assets/images/learning/bigdata/flink/flink-development-environment-setup/25badd064003b99635aac185d59e8efe.png)
...
![IDEA Maven Flink Archetype](/assets/images/learning/bigdata/flink/flink-development-environment-setup/6e63b9414a26088b6bfcd3a1b23af009.png)
...
![添加 Archetype](/assets/images/learning/bigdata/flink/flink-development-environment-setup/8bf827147e0bcb20c94e1a523b38df4f.png)
...
![项目结构](/assets/images/learning/bigdata/flink/flink-development-environment-setup/1b9e3d8529ea5a3516aaa4bd9f5d9ff1.png)

其中 BatchJob 为批处理的样例代码，源码如下：

```scala
import org.apache.flink.api.scala._
object BatchJob {  def main(args: Array[String]) {    val env = ExecutionEnvironment.getExecutionEnvironment
...
object StreamingJob {  def main(args: Array[String]) {    val env = StreamExecutionEnvironment.getExecutionEnvironment
...
```xml
<!-- Apache Flink dependencies --><!-- These dependencies are provided, because they should not be packaged into the JAR file. --><dependency>
...
<!-- Scala Library, provided by Flink as well. --><dependency>
...
![flink-dist jar](/assets/images/learning/bigdata/flink/flink-development-environment-setup/ec4707072be29bd972b4765c170faf4f.png)

scope 标签被标识为 provided 会导致你在 IDEA 中启动项目时会抛出 ClassNotFoundException 异常。基于这个原因，在使用 IDEA 创建项目时还自动生成了以下 profile 配置：

```xml
<!-- This profile helps to make things run out of the box in IntelliJ --><!-- Its adds Flink's core classes to the runtime class path. --><!-- Otherwise they are missing in IntelliJ, because the dependency is 'provided' --><profiles>
  <profile>
...
![Maven profile配置](/assets/images/learning/bigdata/flink/flink-development-environment-setup/d630ce85880ff23728f5441a3ca0a7ac.png)
...
```scala
import org.apache.flink.api.scala._
object WordCountBatch {  def main(args: Array[String]): Unit = {    val benv = ExecutionEnvironment.getExecutionEnvironment
    val dataSet = benv.readTextFile("D:\\wordcount.txt")    dataSet.flatMap { _.toLowerCase.split(",")}    .filter (_.nonEmpty)    .map { (_, 1) }    .groupBy(0)    .sum(1)    .print()  }}
```
...
![批处理结果](/assets/images/learning/bigdata/flink/flink-development-environment-setup/bdf7c2250948b658b8d441fce3f0ef8c.png)

### 4.2 流处理示例

```scala
import org.apache.flink.streaming.api.scala._
import org.apache.flink.streaming.api.windowing.time.Timeobject WordCountStreaming {  def main(args: Array[String]): Unit = {    val senv = StreamExecutionEnvironment.getExecutionEnvironment
    val dataStream: DataStream[String] = senv.socketTextStream("192.168.0.229", 9999, '\n')    dataStream.flatMap { line => line.toLowerCase.split(",") }    .filter(_.nonEmpty)    .map { word => (word, 1) }    .keyBy(0)    .timeWindow(Time.seconds(3))    .sum(1)    .print()    senv.execute("Streaming WordCount")  }}
```
...
![Flink版本下载](/assets/images/learning/bigdata/flink/flink-development-environment-setup/52684210fee5db2c3cd2e375e9f1bf44.png)
...
![Scala Shell示例](/assets/images/learning/bigdata/flink/flink-development-environment-setup/52ff99d8d4455fbecc19d80409608736.png)
