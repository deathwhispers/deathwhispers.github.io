...
layout: post
title: HBase 容灾与备份
slug: hbase-disaster-recovery-and-backup
type:
  - note
date: 2020-03-28
status: draft
...
- CopyTable
的操作是基于 HBase Client API 进行的，即采用
scan
进行查询, 采用
put
进行写入。
...
![](https://cdn.nlark.com/yuque/0/2023/png/29230873/1698027551372-b3f3e0c3-2e36-45a5-8257-fddf3db15663.png)
...
支持从 HDFS 导入数据。
Export
还支持指定导出数据的开始时间和结束时间，因此可以用于增量备份。
- Export
导出与
CopyTable
一样，依赖 HBase 的
...
- 导出的
outputdir
目录可以不用预先创建，程序会自动创建。导出完成后，导出文件的所有权将由执行导出命令的用户所拥有。
- 默认情况下，仅导出给定
Cell
的最新版本，而不管历史版本。要导出多个版本，需要将
参数替换为所需的版本数。
...
6. [Online Apache HBase Backups with CopyTable](https://blog.cloudera.com/blog/2012/06/online-hbase-backups-with-copytable-2/)
7. [Apache HBase ™ Reference Guide](http://hbase.apache.org/book.htm)