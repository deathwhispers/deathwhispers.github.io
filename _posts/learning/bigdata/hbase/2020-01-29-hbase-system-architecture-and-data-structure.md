---
layout: post
title: HBase 系统架构及数据结构
slug: hbase-system-architecture-and-data-structure
type:
  - note
date: 2020-01-29
status: draft
...
![](https://cdn.nlark.com/yuque/0/2023/png/29230873/1698027554630-6ed39b63-114f-4a45-9550-fa07e44346df.png)
...
![](https://cdn.nlark.com/yuque/0/2023/png/29230873/1698027554706-afb458bd-e15e-49f8-9bd2-8f8c19bd8b4c.png)

每个表一开始只有一个 Region，随着数据不断增加，Region 会不断增大，当增大到一个阀值的时候，Region 就会等分为两个新的 Region。当 Table 中的行不断增多，就会有越来越多的 Region。

![](https://cdn.nlark.com/yuque/0/2023/png/29230873/1698027554787-3b8c3eb7-4f6f-4e70-89d5-1f5f60c4daea.png)

Region 是 HBase 中**分布式存储和负载均衡的最小单元**。这意味着不同的 Region 可以分布在不同的 Region Server 上。但一个 Region 是不会拆分到多个 Server 上的。

![](https://cdn.nlark.com/yuque/0/2023/png/29230873/1698027554873-cdc40c14-534d-4f22-91c4-17563046dd00.png)
...
：用于存储尚未进持久化存储的数据记录，以便在发生故障时进行恢复。
- **BlockCache**
：读缓存。它将频繁读取的数据存储在内存中，如果存储不足，它将按照
最近最少使用原则
清除多余的数据。
- **MemStore**
：写缓存。它存储尚未写入磁盘的新数据，并会在数据写入磁盘之前对其进行排序。每个 Region 上的每个列族都有一个 MemStore。
- **HFile**
：将行数据按照 Key的形式存储在文件系统上。

![](https://cdn.nlark.com/yuque/0/2023/png/29230873/1698027554947-899e40f1-9363-4bdb-9c5f-a1edef2de2fc.png)
...
![](https://cdn.nlark.com/yuque/0/2023/png/29230873/1698027555037-15a165f8-2032-4c1a-afc2-2e18bb34dc00.png)
...
5. 为 Region Server 分配 Region ；
6. 负责 Region Server 的负载均衡 ；
7. 发现失效的 Region Server 并重新分配其上的 Region；
8. GFS 上的垃圾文件回收；
9. 处理 Schema 的更新请求。

**Region Server**

10. Region Server 负责维护 Master 分配给它的 Region ，并处理发送到 Region 上的 IO 请求；
11. Region Server 负责切分在运行过程中变得过大的 Region。

![](https://cdn.nlark.com/yuque/0/2023/png/29230873/1698027555128-76ae910d-818c-45eb-9100-a124f7f22a88.png)
...
![](https://cdn.nlark.com/yuque/0/2023/png/29230873/1698027555204-d6a9d612-236f-4da3-a8cf-f2735ef5a6ec.png)

## 四、数据的读写流程简述

### 4.1 写入数据的流程

12. Client 向 Region Server 提交写请求；
13. Region Server 找到目标 Region；
14. Region 检查数据是否与 Schema 一致；
15. 如果客户端没有指定版本，则获取当前系统时间作为数据版本；
16. 将更新写入 WAL Log；
17. 将更新写入 Memstore；
18. 判断 Memstore 存储是否已满，如果存储已满则需要 flush 为 Store Hfile 文件。
...
19. 客户端从 Zookeeper 获取
META
表所在的 Region Server；
20. 客户端访问
META
表所在的 Region Server，从
META
表中查询到访问行键所在的 Region Server，之后客户端将缓存这些信息以及
META
表的位置；
21. 客户端从行键所在的 Region Server 上获取数据。

如果再次读取，客户端将从缓存中获取行键所在的 Region Server。这样客户端就不需要再次查询 META 表，除非 Region 移动导致缓存失效，这样的话，则将会重新查询并更新缓存。

注：META 表是 HBase 中一张特殊的表，它保存了所有 Region 的位置信息，META 表自己的位置信息则存储在 ZooKeeper 上。

![](https://cdn.nlark.com/yuque/0/2023/png/29230873/1698027555287-31343d55-d2b2-4184-9b65-dba26121209c.png)
