...
常用的[Headers (opens new window)](https://github.com/jetlinks/jetlinks-core/blob/master/src/main/java/org/jetlinks/core/message/Headers.java):
...
当前分片索引
6. **frag_last**
是否为最后一个分片,当无法确定分片数量的时候,可以将分片设置到足够大,最后一个分片设置:
frag_last=true
来完成返回.
7. **keepOnline**
与
DeviceOnlineMessage
配合使用,在TCP短链接,保持设备一直在线状态,连接断开不会设置设备离线.
8. **keepOnlineTimeoutSeconds**
指定在线超时时间,在短链接时,如果超过此间隔没有收到消息则认为设备离线.
9. **ignoreStorage**
不存储此消息数据,如: 读写属性回复默认也会记录到属性时序数据库中,设置为true后,将不记录.(1.9版本后支持)
10. **ignoreLog**
不记录此消息到日志,如: 设置为true,将不记录此消息的日志.
11. **mergeLatest**[开启最新数据存储](http://doc.jetlinks.cn/best-practices/start.html#%E8%AE%B0%E5%BD%95%E8%AE%BE%E5%A4%87%E6%9C%80%E6%96%B0%E6%95%B0%E6%8D%AE%E5%88%B0%E6%95%B0%E6%8D%AE%E5%BA%93)
是否合并最新属性数据,设置此消息头后,将会把最新的消息合并到消息体里(需要
).
...
12. 获取设备属性(ReadPropertyMessage)
对应设备回复的消息
ReadPropertyMessageReply
.
13. 修改设备属性(WritePropertyMessage)
对应设备回复的消息
WritePropertyMessageReply
.
14. 设备上报属性(ReportPropertyMessage)
由设备上报.
...
15. DeviceOnlineMessage
设备上线消息,通常用于网关代理的子设备的上线操作.
16. DeviceOfflineMessage
设备离线消息,通常用于网关代理的子设备的下线操作.
17. ChildDeviceMessage
子设备消息,通常用于网关代理的子设备的消息.
18. ChildDeviceMessageReply
子设备消息回复,用于平台向网关代理的子设备发送消息后设备回复给平台的结果.
19. UpdateTagMessage
更新设备标签.
20. DerivedMetadataMessage
更新设备独立物模型.
21. DeviceRegisterMessage
设备注册消息,通过设置消息头
message.addHeader(“deviceName”,“设备名称”);
和
message.addHeader(“productId”,“产品ID”)
可实现设备自动注册.
22. 如果配置了状态自管理,在检查子设备状态时,会发送指令
ChildDeviceMessage
, 网关需要返回
...
23. 此topic和mqtt的topic没有任何关系,仅仅作为系统内部通知的方式
24. 使用通配符订阅可能将收到大量的消息,请保证消息的处理速度,否则会影响系统消息吞吐量.
...
![](/assets/img/device-flow.ab1da967.svg)
...
27. 在确定好存储方案后,尽量不要跨类型进行修改,如将: 行式存储修改为列式存储,可能会导致数据结构错乱.
28. 在创建物模型时,请根据存储策略判断好是否支持此类型以及ID格式

索引结构:

29. 属性索引模版:
properties_{productId}_template
...
30. 事件索引模版:
event_{productId}_{eventId}_template
...
31. 设备日志索引模版:
device_log_{productId}_template
...
34. 属性:
properties_{productId}
35. 事件:
event_{productId}_{eventId}
36. 日志:
...
43. 设备属性: (deviceId,property,partition)
44. 设备事件: (deviceId,partition)
45. 设备日志: (deviceId,partition)
...
46. 在查询条件没有设置时间的时候，默认只会查询最近2个分区周期内的数据。 前端在查询时，建议默认指定时间范围，并且时间间隔不能与配置的分区周期相差太多，否则会影响查询性能.
47. Cassandra仅支持使用滚动分页,跳转分页深度过大会降低查询速度.执行count的性能也较低,建议查询时,使用[滚动分页](http://doc.jetlinks.cn/interface-guide/query-param.html#%E6%BB%9A%E5%8A%A8%E5%88%86%E9%A1%B5)
.
